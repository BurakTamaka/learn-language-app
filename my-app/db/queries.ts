    ...
    const normalizedData = data.map((unit) => {
        const lessonWithCompletedStatus = unit.lessons.map((lesson)=> {
            if(lesson.challenges.length === 0){
                return{...lesson,completed:false}  // ilerleme durumunu gösterir
            }
            const allCompletedChalleges = lesson.challenges.every((challenge) => {
            ...
            return {...lesson, completed : allCompletedChalleges}
        })
